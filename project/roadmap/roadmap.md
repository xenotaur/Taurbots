...
2. **Kubuntu workstation diagnostics and setup docs**: document and later implement `taurbots doctor`-style checks for SSH/byobu/screen workflow, GPU readiness, Docker/NVIDIA Container Toolkit, optional Apptainer/Singularity, ROS/Gazebo readiness, and remote operation before mutating setup automation.
...
6. **Core abstractions and experiment schema**: specify simulator-independent concepts such as State/Pose/Observation/Action, World/Map/Scenario, RobotModel, Task, Policy/PolicyRunner, SimulatorBackend, RolloutEngine, Roadmap, EdgeEvaluator, Planner, Episode, EvaluationRecord, and ExperimentProfile, then write structured result records for episodes, metrics, seeds, versions, and artifacts.
...
This ordering intentionally follows the adopted design proposal's initial implementation path by putting `taurbots doctor`-style diagnostics before `simple2d`, PRM-RL, structured result-schema work, benchmark recipes, optional Webots/MuJoCo work, ROS 2/Gazebo validation, and later MCP/agentic wrappers. The added core package skeleton stage is a planning/scaffolding prerequisite for safe implementation, not a replacement for the adopted runtime sequence.

## Near-Term Priority

The next implementation PRs should prefer the first four stages: planning alignment, diagnostic-first workstation documentation, core skeleton conventions, and the `simple2d` smoke-test path. Core schema work can proceed in parallel with the smoke-test and PRM-RL path, but workstation and benchmark dependencies must not contaminate the simulator-independent core.

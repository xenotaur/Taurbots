...
6. **Kubuntu workstation diagnostics and setup docs**: document and later implement `taurbots doctor`-style checks for SSH/byobu/screen workflow, GPU readiness, Docker/NVIDIA Container Toolkit, optional Apptainer/Singularity, and ROS/Gazebo readiness.
...
The next implementation PRs should prefer the first three stages: skeleton conventions, core abstractions, and experiment/result schema. Workstation and benchmark work can proceed in parallel as documentation and diagnostics, but must not contaminate the simulator-independent core.
